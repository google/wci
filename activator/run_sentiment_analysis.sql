...
WITH
  ...
  SELECT
    ...
    CONCAT('From 0 to 1, how satisfied is this customer? ', ARRAY_TO_STRING(ARRAY_AGG(message
        ORDER BY
          TIMESTAMP ASC), '\n')) AS prompt,
  ...
  FROM
    journeys
  LEFT JOIN
    `$GOOGLE_CLOUD_PROJECT.$BQ_DATASET_NAME.sentiment_analysis` AS storaged
  USING
    (chat_id)
  WHERE
